<template>
  <div class="min-h-screen">
    <div class="">
      ...
      <!-- Filter Section -->
      <div class="border border-gray-800 rounded-lg shadow-sm p-6 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            ...
            <select v-model="selectedStatus" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              <option class="bg-elevated" value="success">Success</option>
              <option class="bg-elevated" value="rejected">Rejected</option>
            </select>
          </div>
          <div>
            <label class="block text-sm font-medium mb-2">Tipe Transfer</label>
            <select v-model="selectedType" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option class="bg-elevated" value="">Semua Tipe</option>
              <option class="bg-elevated" value="my_requests">Permintaan Saya</option>
              <option class="bg-elevated" value="incoming_requests">Permintaan Masuk</option>
            </select>
          </div>
          <!-- <div>
        ...
        <div class="mt-4 flex justify-between items-center">
          <div class="flex space-x-2">
            <button
              @click="resetFilters"
              class="px-4 py-2 bg-primary-600 rounded-md hover:bg-primary-700 transition-colors"
            >
              Reset Filter
            </button>
          </div>
  ...
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
    <div>
      ...
      <p class="text-sm text-gray-600 dark:text-gray-400">
        Download laporan riwayat transfer stock dalam format PDF per bulan
      </p>
    </div>

    <!-- Komponen PDF Export -->
    <TransferStockPDFExport
      :transfer-data="allTransfers"
    ...
    />
      ...
      <!-- Data Table -->
      <div v-else class="border border-gray-800 rounded-lg shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
          <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="(transfer, index) in paginatedTransfers" :key="transfer.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-200" :class="{ 'bg-blue-50/30 dark:bg-blue-900/10': index % 2 === 0 }">
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  <div>
                    ...
                    <div class="text-info">{{ transfer.unit_request.product_type.product_name_type }}</div>
                  </div>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        ...
                        {{ formatDate(transfer.created_at).split(' ').slice(-1)[0] }}
                </td>
        ...
        <!-- Pagination -->
        <div v-if="filteredTransfers.length > 0" class="px-4 py-3 flex items-center justify-between border-t border-gray-800 sm:px-6">
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px">
                <button
                  @click="currentPage--"
                  :disabled="currentPage === 1"
                  class="relative inline-flex items-center px-2 py-2 rounded-l-md border border-primary-800 text-sm font-medium text-primary hover:bg-elevated disabled:opacity-50"
                >
                ...
                <button
                  ...
                  :class="[
                    ...
                    page === currentPage
                      ? 'z-10 bg-elevated border-primary-800 text-primary-600'
                      : ' border-primary-300 text-elevated hover:bg-elevated'
                  ]"
                >
                  {{ page }}
                </button>
                ...
                <button
                  @click="currentPage++"
                  :disabled="currentPage === totalPages"
                  class="relative inline-flex items-center px-2 py-2 rounded-r-md border border-primary-800 text-sm font-medium text-primary hover:bg-elevated disabled:opacity-50"
                >
...
const availableYears = computed(() => {
  ...
  allTransfers.value.forEach((item) => {
    const year = new Date(item.created_at).getFullYear()
    years.add(year)
  })
  return Array.from(years).sort((a, b) => b - a) as number[]
})
