...
import { useSalesStore } from '~/store/sales-detail' // Import sales detail store
import { sub, isWithinInterval, parseISO, startOfDay, endOfDay, getYear } from 'date-fns'
import type { Period, Range } from '~/types'

const salesOrderStore = useSalesOrderStore()
const salesDetailStore = useSalesStore() // Inisialisasi sales detail store
...
const selectedYear = ref(new Date().getFullYear()) // Filter tahun untuk total pendapatan
const itemsPerPage = ref(10)

const paginatedFilteredSalesOrders = computed(() => {
...
const filteredSalesOrders = computed(() => {
  let filtered = [...(salesOrderStore.salesOrderList || [])]
  ...
  if (searchQuery.value) {
    filtered = filtered.filter((item: SalesOrder) =>
      (item.id.toString().includes(searchQuery.value)) ||
      (item.id_user.toString().includes(searchQuery.value)) ||
      (item.username?.toLowerCase().includes(searchQuery.value.toLowerCase()) ?? false) ||
...
const totalOrders = computed(() => {
  return filteredSalesOrders.value.length
})

// Computed untuk total revenue berdasarkan tahun yang dipilih
const totalRevenue = computed(() => {
  return salesOrderStore.salesOrderList
    .filter((item) => {
...
// Computed untuk filtered revenue
const filteredRevenue = computed(() => {
  return filteredSalesOrders.value.reduce((sum, item) => {
    return sum + Number(item.total_amount || 0)
  }, 0)
})
...
const availableYears = computed(() => {
  ...
  salesOrderStore.salesOrderList.forEach((item) => {
...
const showDetail = (salesOrderId: number) => {
  router.push(`/kasir/transaksi/riwayat-show/${salesOrderId}`)
}
...
onMounted(() => {
  // Hanya fetch sales orders, total sudah dihitung di backend
...
<template>
  <UDashboardPanel id="akunkasir" :ui="{ body: 'lg:py-12' }">
    <template #header>
      <UDashboardNavbar title="Riwayat Penjualan">
        <template #leading>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div class="bg-gradient-to-r from-info-500 to-info-600 rounded-xl p-6">
    ...
    <!-- Filter dan Search -->
    <div class="rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        ...
        <div class="flex-1">
          ...
          <div class="relative">
            ...
            <input
              v-model="searchQuery"
              type="text"
              placeholder="Cari berdasarkan ID, Username"
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
        <span>Menampilkan {{ filteredSalesOrders.length }} transaksi</span>
        <span v-if="searchQuery || (range.start && range.end)">
          Total filtered: {{ formatCurrency(filteredRevenue) }}
        </span>
      </div>
    </div>
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p class="text-sm text-gray-600 dark:text-gray-400">
            Download laporan penjualan dalam format PDF per bulan
          </p>
        </div>

        <!-- Komponen PDF Export -->
        <SalesPDFExport
          :sales-orders="salesOrderStore.salesOrderList || []"
    ...
    <div class="rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
      ...
      <div v-if="salesOrderStore.isLoading || salesDetailStore.isLoading" class="flex justify-center items-center py-12">
        <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-500"></div>
      </div>

      <div v-else-if="salesOrderStore.error" class="text-center py-12">
        ...
        <p class="text-red-500 dark:text-red-400">{{ salesOrderStore.error }}</p>
      </div>

      <div v-else-if="filteredSalesOrders.length === 0" class="text-center py-12">
        ...
        <p class="text-gray-500 dark:text-gray-400">
          {{ searchQuery || (range.start && range.end) ? 'Tidak ada transaksi yang sesuai dengan filter' : 'Belum ada transaksi yang tercatat' }}
        </p>
      </div>
        ...
        <table class="w-full">
          <thead>
            <tr class="border-b border-gray-200 dark:border-gray-600">
              ...
              <th class="text-left px-6 py-4 text-sm font-semibold">Nama</th>
              <th class="text-left px-6 py-4 text-sm font-semibold">Tanggal</th>
              <th class="text-right px-6 py-4 text-sm font-semibold">Total</th>
              <th class="text-center px-6 py-4 text-sm font-semibold">Aksi</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr
              v-for="(item, index) in paginatedFilteredSalesOrders"
              :key="item.id"
            ...
            >
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <span class="font-medium text-gray-900 dark:text-white">{{ item.username || 'Unknown User' }}</span>
                </div>
              </td>
              <td class="px-6 py-4 text-gray-700 dark:text-gray-300">
                {{ new Date(item.date).toLocaleDateString('id-ID') }}
                {{ formatDateTime(item.date).split(' ').slice(-1)[0] }}
              </td>
              ...
              <td class="px-6 py-4 text-center">
                <button
                  ...
                  class="inline-flex items-center px-3 py-2 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 dark:bg-primary-500 dark:hover:bg-primary-600 rounded-lg transition-colors duration-200 gap-2"
                >
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
