    ...
    >
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 30px; padding: 0 20px;">
        <div style="border: 2px solid #3b82f6; border-radius: 8px; padding: 20px; text-align: center;">
          <h3 style="font-size: 14px; color: #6b7280; margin-bottom: 10px;">Total Transaksi</h3>
        ...
        <div style="border: 2px solid #10b981; border-radius: 8px; padding: 20px; text-align: center;">
          <h3 style="font-size: 14px; color: #6b7280; margin-bottom: 10px;">Total Pendapatan</h3>
          <p style="font-size: 24px; font-weight: bold; color: #1f2937;">{{ formatCurrency(totalRevenue) }}</p>
        </div>
      </div>
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 12px;">
          <thead>
            <tr style="background-color: #f3f4f6;">
              ...
              <th style="border: 1px solid #d1d5db; padding: 12px 8px; text-align: left; font-weight: bold;">ID Transaksi</th>
              <th style="border: 1px solid #d1d5db; padding: 12px 8px; text-align: left; font-weight: bold;">Nama Customer</th>
          ...
          <tbody>
            <tr v-for="(item, index) in pdfData" :key="item.id" :style="{ backgroundColor: index % 2 === 0 ? '#f9fafb' : 'white' }">
              ...
              <td style="border: 1px solid #d1d5db; padding: 8px; text-align: left;">SO-{{ item.id }}</td>
              <td style="border: 1px solid #d1d5db; padding: 8px; text-align: left;">{{ item.username || 'Unknown User' }}</td>
...
const pdfData = computed(() => {
  return props.salesOrders.filter(item => {
...
const generatePDF = async () => {
  ...
  try {
    ...
    // Save PDF
    const fileName = `Laporan-Penjualan-${formatMonthYear(selectedMonth.value, selectedYear.value).replace(' ', '-')}.pdf`
    pdf.save(fileName)
